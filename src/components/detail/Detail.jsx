...
export function Detail(){
  ...
  useEffect(()=>{
    const getDetail = async()=> {
        const detail =  await  tmdApi.detail(category.movie, idMovie);
        const credits =  await  tmdApi.credits(category.movie, idMovie);
        setMovie(detail);
        setCredits(credits);
    }
  ...
  const url = apiConfig.originalImage(movie.backdrop_path?movie.backdrop_path : movie.poster_path);
  const imageUrl = apiConfig.w300Image(movie.poster_path?movie.poster_path : movie.backdrop_path);
 
  return (<div className="detail-background" style={{backgroundImage: `url(${url})`}} >
        <div className="detail-container">
                ...
                <div className="genres" >
                    {
                        movie.genres&& movie.genres.map(genre=>{
                            return <ButtonOutline  texto={genre.name} />
                        })
                    }    
                </div>  
            </div>
            ...
            {
                credits.cast&& credits.cast.slice(0,8).map(cast=>{
